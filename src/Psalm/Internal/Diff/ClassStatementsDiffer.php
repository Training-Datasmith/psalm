 ...
 * @internal
 */
final class Class_Statements_Differ extends Ast_Differ
{
    /**
    ...
    {
        $diff_map = [];
        [$trace, $x, $y, $bc] = self::calculate_trace(static function (Php_Parser\Node\Stmt $a, Php_Parser\Node\Stmt $b, string $a_code, string $b_code, bool &$body_change = false) use (&$diff_map): bool {
            if ($a::class !== $b::class) {
                return false;
            }
            ...
            $a_comments = $a->get_comments();
            /** @var list<PhpParser\Comment> */
            $b_comments = $b->get_comments();
            ...
            if ($a_comments) {
                ...
                $a_start = $a_comments[0]->get_start_file_pos();
            }
            if ($b_comments) {
                if (!$a_comments) {
                    $signature_change = true;
                }
                $b_start = $b_comments[0]->get_start_file_pos();
            }
            ...
            if ($a_size === $b_size && substr($a_code, $a_start, $a_size) === substr($b_code, $b_start, $b_size)) {
                ...
                $line_diff = $b->get_line() - $a->get_line();
            ...
            if ($a instanceof Php_Parser\Node\Stmt\Class_Method && $b instanceof Php_Parser\Node\Stmt\Class_Method) {
                if ((string) $a->name !== (string) $b->name) {
                    return false;
                }
                if ($a->stmts) {
                    $first_stmt = $a->stmts[0];
                    $a_stmts_start = (int) $first_stmt->get_attribute('startFilePos');
                    if ($a_stmt_comments = $first_stmt->get_comments()) {
                        $a_stmts_start = $a_stmt_comments[0]->get_start_file_pos();
                    }
                } else {
                    $a_stmts_start = $a_end;
                }
                if ($b->stmts) {
                    ...
                    $b_stmts_start = (int) $first_stmt->get_attribute('startFilePos');
                    if ($b_stmt_comments = $first_stmt->get_comments()) {
                        $b_stmts_start = $b_stmt_comments[0]->get_start_file_pos();
                    }
                } else {
                    $b_stmts_start = $b_end;
                }
                ...
                if (!$signature_change) {
                    ...
                    if ($a_signature !== $b_signature) {
                        ...
                        if (!str_contains($a_signature, $b_signature) && !str_contains($b_signature, $a_signature)) {
                            $signature_change = true;
                        }
                    }
                }
            } elseif ($a instanceof Php_Parser\Node\Stmt\Property && $b instanceof Php_Parser\Node\Stmt\Property) {
                ...
                if ($a->type && $b->type) {
                    $a_type_start = (int) $a->type->get_attribute('startFilePos');
                    $a_type_end = (int) $a->type->get_attribute('endFilePos');
                    $b_type_start = (int) $b->type->get_attribute('startFilePos');
                    $b_type_end = (int) $b->type->get_attribute('endFilePos');
                    if (substr($a_code, $a_type_start, $a_type_end - $a_type_start + 1) !== substr($b_code, $b_type_start, $b_type_end - $b_type_start + 1)) {
                        return false;
                    }
                }
            ...
            if (!$signature_change && !$body_change) {
                $diff_map[] = [$a_start, $a_end, $b_start - $a_start, $b->get_line() - $a->get_line()];
            }
        ...
        foreach ($diff as $diff_elem) {
            if ($diff_elem->type === Diff_Elem::TYPE_KEEP) {
                if ($diff_elem->old instanceof Php_Parser\Node\Stmt\Class_Method) {
                    $keep[] = $name_lc . '::' . strtolower((string) $diff_elem->old->name);
                } elseif ($diff_elem->old instanceof Php_Parser\Node\Stmt\Property) {
                    foreach ($diff_elem->old->props as $prop) {
                        $keep[] = $name_lc . '::$' . $prop->name;
                    }
                } elseif ($diff_elem->old instanceof Php_Parser\Node\Stmt\Class_Const) {
                    foreach ($diff_elem->old->consts as $const) {
                        $keep[] = $name_lc . '::' . $const->name;
                    }
                } elseif ($diff_elem->old instanceof Php_Parser\Node\Stmt\Trait_Use) {
                    foreach ($diff_elem->old->traits as $trait) {
                        $keep[] = $name_lc . '&' . strtolower((string) $trait->get_attribute('resolvedName'));
                    }
                }
            } elseif ($diff_elem->type === Diff_Elem::TYPE_KEEP_SIGNATURE) {
                if ($diff_elem->old instanceof Php_Parser\Node\Stmt\Class_Method) {
            ...
            } elseif ($diff_elem->type === Diff_Elem::TYPE_REMOVE || $diff_elem->type === Diff_Elem::TYPE_ADD) {
                /** @var PhpParser\Node */
                $affected_elem = $diff_elem->type === Diff_Elem::TYPE_REMOVE ? $diff_elem->old : $diff_elem->new;
                if ($affected_elem instanceof Php_Parser\Node\Stmt\Class_Method) {
                    $method_name = strtolower((string) $affected_elem->name);
                    $add_or_delete[] = $name_lc . '::' . $method_name;
                    if ($method_name === '__construct') {
                        foreach ($affected_elem->get_params() as $param) {
                            if (!$param->flags) {
                                continue;
                            }
                ...
                } elseif ($affected_elem instanceof Php_Parser\Node\Stmt\Property) {
                    foreach ($affected_elem->props as $prop) {
                        $add_or_delete[] = $name_lc . '::$' . $prop->name;
                    }
                } elseif ($affected_elem instanceof Php_Parser\Node\Stmt\Class_Const) {
                    foreach ($affected_elem->consts as $const) {
                        $add_or_delete[] = $name_lc . '::' . $const->name;
                    }
                } elseif ($affected_elem instanceof Php_Parser\Node\Stmt\Trait_Use) {
                    foreach ($affected_elem->traits as $trait) {
                        $add_or_delete[] = $name_lc . '&' . strtolower((string) $trait->get_attribute('resolvedName'));
                    }
                }
                if ($diff_elem->type === Diff_Elem::TYPE_REMOVE) {
                    if ($doc = $affected_elem->get_doc_comment()) {
                        $start = $doc->get_start_file_pos();
                    } else {
                        $start = (int) $affected_elem->get_attribute('startFilePos');
                    }
                    $deletion_ranges[] = [$start, (int) $affected_elem->get_attribute('endFilePos')];
                }
            }
        }
