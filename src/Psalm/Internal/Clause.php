...
final class Clause implements Stringable
{
     ...
     */
    public function __construct(array $possibilities, public int $creating_conditional_id, int $creating_object_id, bool $wedge = false, bool $reconcilable = true, public bool $generated = false, public array $redefined_vars = [])
    {
    ...
    public function remove_possibilities(string $var_id): ?self
    {
     ...
     */
    public function add_possibilities(string $var_id, array $clause_var_possibilities): self
    {
        $possibilities = $this->possibilities;
        $possibilities[$var_id] = $clause_var_possibilities;
        return new self($possibilities, $this->creating_conditional_id, $this->creating_object_id, $this->wedge, $this->reconcilable, $this->generated, $this->redefined_vars);
    }
    public function calculate_negation(): self
    {
        ...
        foreach ($this->possibilities as $var_id => $possibility) {
            ...
            foreach ($possibility as $type) {
                if (!$type->has_equality() || ($inner_type = $type->get_atomic_type()) && ($inner_type instanceof T_Literal_Int || $inner_type instanceof T_Literal_Float || $inner_type instanceof T_Literal_String || $inner_type instanceof T_Class_Constant || $inner_type instanceof T_Enum_Case)) {
                    $impossibility[] = $type->get_negation();
                }
            }
            if ($impossibility) {
