 ...
 * @internal
 */
final class Type_Tokenizer
{
    /**
     * @var array<string, bool>
     */
    public const PSALM_RESERVED_WORDS = ['int' => true, 'string' => true, 'float' => true, 'bool' => true, 'false' => true, 'true' => true, 'object' => true, 'empty' => true, 'callable' => true, 'array' => true, 'non-empty-array' => true, 'non-empty-string' => true, 'non-falsy-string' => true, 'truthy-string' => true, 'iterable' => true, 'null' => true, 'mixed' => true, 'numeric-string' => true, 'class-string' => true, 'interface-string' => true, 'enum-string' => true, 'trait-string' => true, 'callable-string' => true, 'callable-array' => true, 'callable-object' => true, 'stringable-object' => true, 'pure-callable' => true, 'pure-Closure' => true, 'literal-string' => true, 'non-empty-literal-string' => true, 'lowercase-string' => true, 'non-empty-lowercase-string' => true, 'positive-int' => true, 'non-negative-int' => true, 'negative-int' => true, 'non-positive-int' => true, 'literal-int' => true, 'boolean' => true, 'integer' => true, 'double' => true, 'real' => true, 'resource' => true, 'void' => true, 'self' => true, 'static' => true, 'scalar' => true, 'numeric' => true, 'no-return' => true, 'never-return' => true, 'never-returns' => true, 'never' => true, 'array-key' => true, 'key-of' => true, 'value-of' => true, 'properties-of' => true, 'public-properties-of' => true, 'protected-properties-of' => true, 'private-properties-of' => true, 'non-empty-countable' => true, 'list' => true, 'non-empty-list' => true, 'class-string-map' => true, 'open-resource' => true, 'closed-resource' => true, 'associative-array' => true, 'arraylike-object' => true, 'int-mask' => true, 'int-mask-of' => true];
    /**
    ...
    public static function tokenize(string $string_type, bool $ignore_space = true): array
    {
        ...
        for ($i = 0, $c = count($chars); $i < $c; ++$i) {
            ...
            if ($was_space && ($char === '$' || $char === '.' && ($chars[$i + 1] ?? null) === '.' && ($chars[$i + 2] ?? null) === '.' && ($chars[$i + 3] ?? null) === '$')) {
                $type_tokens[++$rtc] = [' ', $i - 1];
                $type_tokens[++$rtc] = ['', $i];
            } elseif ($was_space && in_array(implode('', array_slice($chars, $i, 3)), ['as ', 'is ', 'of '])) {
            ...
            if ($char === '.') {
                ...
                if ($i + 2 > $c || $chars[$i + 1] !== '.' || $chars[$i + 2] !== '.') {
                    throw new Type_Parse_Tree_Exception('Unexpected token ' . $char);
                }
                if ($type_tokens[$rtc][0] === '') {
                    $type_tokens[$rtc] = ['...', $i];
                } else {
     ...
     * @psalm-pure
     */
    public static function fix_scalar_terms(string $type_string, ?int $analysis_php_version_id = null): string
    {
     ...
     */
    public static function get_fully_qualified_tokens(string $string_type, Aliases $aliases, ?array $template_type_map = null, ?array $type_aliases = null, ?string $self_fqcln = null, ?string $parent_fqcln = null, bool $allow_assertions = false): array
    {
        $type_tokens = self::tokenize($string_type);
        for ($i = 0, $l = count($type_tokens); $i < $l; ++$i) {
            $string_type_token = $type_tokens[$i];
            if (in_array($string_type_token[0], ['<', '>', '|', '?', ',', '{', '}', ':', '::', '[', ']', '(', ')', '&', '=', '...', 'as', 'is'], true)) {
                continue;
            }
            if ($string_type_token[0][0] === '\\' && strlen((string) $string_type_token[0]) === 1) {
                throw new Type_Parse_Tree_Exception("Backslash \"\\\" has to be part of class name.");
            }
            ...
            $fixed_token = !isset($type_tokens[$i + 1]) || $type_tokens[$i + 1][0] !== '(' ? self::fix_scalar_terms($string_type_token[0]) : $string_type_token[0];
            ...
            if (isset($type_aliases[$string_type_token[0]])) {
                ...
                if ($type_alias instanceof Inline_Type_Alias) {
                ...
                $type_tokens[$i][0] = Type::get_fqcln_from_string($string_type_token[0], $aliases);
            }
        }
        return $type_tokens;
    }
