 ...
 * @internal
 */
final class Integer_Range_Comparator
{
    /**
     * This method is used to check if an integer range can be contained in another
     */
    public static function is_contained_by(T_Int_Range $input_type_part, T_Int_Range $container_type_part): bool
    {
     ...
     */
    public static function is_contained_by_union(T_Int_Range $input_type_part, Union $container_type): bool
    {
        $container_atomic_types = $container_type->get_atomic_types();
        $reduced_range = new T_Int_Range($input_type_part->min_bound, $input_type_part->max_bound, $input_type_part->from_docblock);
        if (isset($container_atomic_types['int'])) {
            if ($container_atomic_types['int']::class === T_Int::class) {
                return true;
            }
            if ($container_atomic_types['int']::class === T_Nonspecific_Literal_Int::class) {
                return true;
            }
        ...
        do {
            ...
            $result_reduction = self::reduce_range_incrementally($container_atomic_types, $reduced_range);
            $new_nb_atomics = count($container_atomic_types);
     ...
     */
    private static function reduce_range_incrementally(array &$container_atomic_types, T_Int_Range $reduced_range): ?bool
    {
        foreach ($container_atomic_types as $key => $container_atomic_type) {
            if ($container_atomic_type instanceof T_Int_Range) {
                if (self::is_contained_by($reduced_range, $container_atomic_type)) {
                    ...
                    if ($container_atomic_type->max_bound === null) {
                        ...
                        $reduced_range->max_bound = T_Int_Range::get_new_lowest_bound($container_atomic_type->min_bound - 1, $reduced_range->max_bound ?? $container_atomic_type->min_bound - 1);
                    ...
                    if ($container_atomic_type->min_bound === null) {
                        ...
                        $reduced_range->min_bound = T_Int_Range::get_new_highest_bound($container_atomic_type->max_bound + 1, $reduced_range->min_bound ?? $container_atomic_type->max_bound + 1);
                        unset($container_atomic_types[$key]);
                        //we don't need this one anymore
                        continue;
                    }
            ...
            } elseif ($container_atomic_type instanceof T_Literal_Int) {
