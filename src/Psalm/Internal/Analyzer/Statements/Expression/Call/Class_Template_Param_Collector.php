 ...
 * @internal
 */
final class Class_Template_Param_Collector
{
    /**
     ...
     */
    public static function collect(Codebase $codebase, Class_Like_Storage $class_storage, Class_Like_Storage $static_class_storage, ?string $method_name = null, ?Atomic $lhs_type_part = null, bool $self_call = false): ?array
    {
        ...
        if ($static_class_storage->template_extended_params && $method_name && !empty($non_trait_class_storage->overridden_method_ids[$method_name])) {
        ...
        if ($lhs_type_part instanceof T_Generic_Object) {
            ...
            if ($class_storage !== $static_class_storage && $static_class_storage->template_types) {
                ...
                $template_result = new Template_Result($static_class_storage->template_types, $templates);
            }
            foreach ($template_types as $type_name => $_) {
                ...
                if ($class_storage !== $static_class_storage && isset($e[$class_storage->name][$type_name])) {
                    ...
                    $output_type_extends = self::resolve_template_param($codebase, $input_type_extends, $static_class_storage, $lhs_type_part, $template_result);
                    $class_template_params[$type_name] = [$class_storage->name => $output_type_extends ?? Type::get_mixed()];
                } else {
                    $class_template_params[$type_name] = [$class_storage->name => Type::get_mixed()];
                }
            }
        }
            ...
            foreach ($type_map as $type) {
                foreach ($candidate_class_storages as $candidate_class_storage) {
                    if ($candidate_class_storage !== $static_class_storage && isset($e[$candidate_class_storage->name][$type_name]) && !isset($class_template_params[$type_name][$candidate_class_storage->name])) {
                        $class_template_params[$type_name][$candidate_class_storage->name] = new Union(self::expand_type($codebase, $e[$candidate_class_storage->name][$type_name], $e, $static_class_storage->name, $static_class_storage->template_types));
                    }
                }
                if (!$self_call) {
                    if (!isset($class_template_params[$type_name])) {
    ...
    private static function resolve_template_param(Codebase $codebase, Union $input_type_extends, Class_Like_Storage $static_class_storage, T_Generic_Object $lhs_type_part, ?Template_Result $template_result = null): ?Union
    {
        $output_type_extends = null;
        foreach ($input_type_extends->get_atomic_types() as $type_extends_atomic) {
            if ($type_extends_atomic instanceof T_Template_Param) {
                if (isset($static_class_storage->template_types[$type_extends_atomic->param_name][$type_extends_atomic->defining_class])) {
                    $mapped_offset = array_search($type_extends_atomic->param_name, array_keys($static_class_storage->template_types), true);
                    if ($mapped_offset !== false && isset($lhs_type_part->type_params[$mapped_offset])) {
                        $output_type_extends = Type::combine_union_types($lhs_type_part->type_params[$mapped_offset], $output_type_extends);
                    }
                    ...
                    $nested_output_type = self::resolve_template_param($codebase, $static_class_storage->template_extended_params[$type_extends_atomic->defining_class][$type_extends_atomic->param_name], $static_class_storage, $lhs_type_part, $template_result);
                    if ($nested_output_type !== null) {
                        $output_type_extends = Type::combine_union_types($nested_output_type, $output_type_extends);
                    }
                }
            } else {
                if ($template_result !== null) {
                    $type_extends_atomic = $type_extends_atomic->replace_template_types_with_arg_types($template_result, $codebase);
                }
                $output_type_extends = Type::combine_union_types(new Union([$type_extends_atomic]), $output_type_extends);
            }
        }
     ...
     */
    private static function expand_type(Codebase $codebase, Union $input_type_extends, array $e, string $static_fq_class_name, ?array $static_template_types): array
    {
        $output_type_extends = [];
        foreach ($input_type_extends->get_atomic_types() as $type_extends_atomic) {
            if ($type_extends_atomic instanceof T_Template_Param && ($static_fq_class_name !== $type_extends_atomic->defining_class || !isset($static_template_types[$type_extends_atomic->param_name])) && isset($e[$type_extends_atomic->defining_class][$type_extends_atomic->param_name])) {
                $output_type_extends = [...$output_type_extends, ...self::expand_type($codebase, $e[$type_extends_atomic->defining_class][$type_extends_atomic->param_name], $e, $static_fq_class_name, $static_template_types)];
            } elseif ($type_extends_atomic instanceof T_Class_Constant) {
                $expanded = Type_Expander::expand_atomic($codebase, $type_extends_atomic, $type_extends_atomic->fq_classlike_name, $type_extends_atomic->fq_classlike_name, null, true, true);
