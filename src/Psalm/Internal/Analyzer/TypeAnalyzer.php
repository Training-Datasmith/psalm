...
final class Type_Analyzer
{
     ...
     */
    public static function combine_keyed_types(array $new_types, array $existing_types): array
    {
        ...
        foreach ($keys as $key) {
            ...
            if ($new_var_types->get_id() === $existing_var_types->get_id()) {
                $result_types[$key] = $new_var_types;
            } else {
                $result_types[$key] = Type::combine_union_types($new_var_types, $existing_var_types);
            }
        }
        return $result_types;
    }
