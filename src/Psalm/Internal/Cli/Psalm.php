...
use Fidry\Cpu_Core_Counter\Cpu_Core_Counter;
use Psalm\Config;
...
use Psalm\Internal\Codebase\Reference_Map_Generator;
...
use Psalm\Progress\Long_Progress;
...
final class Psalm
{
    ...
    public static function run(array $argv): void
    {
        ...
        if (!array_key_exists('debug', $options) && 'true' === getenv('GITHUB_ACTIONS') && '1' === getenv('RUNNER_DEBUG')) {
            $options['debug'] = false;
        }
        self::forward_cli_call($options, $argv);
        ...
        $current_dir = self::get_current_dir($options);
        ...
        $run_taint_analysis = self::should_run_taint_analysis($options);
        if (array_key_exists('v', $options)) {
            echo 'Psalm ' . PSALM_VERSION . PHP_EOL;
            exit;
        }
        $output_format = self::init_output_format($options);
        [$config, $init_source_dir] = self::init_config($current_dir, $args, $vendor_dir, $first_autoloader, $path_to_config, $output_format, $run_taint_analysis, $options);
        ...
        $threads = self::get_threads($options, $config, $in_ci, false);
        $scan_threads = self::get_threads($options, $config, $in_ci, true);
        $progress = self::init_progress($options, $config, $in_ci);
        $force_jit = $config->force_jit || isset($options['force-jit']);
        self::restart($options, $force_jit, $threads, $scan_threads, $progress);
        ...
        $show_info = self::init_show_info($options);
        $is_diff = false;
        // self::initIsDiff($options);
        $find_unused_code = self::should_find_unused_code($options, $config);
        ...
        self::configure_shepherd($config, $options, $plugins);
        if (isset($options['clear-cache'])) {
            self::clear_cache($config);
        }
        if (isset($options['consolidate-cache'])) {
            self::consolidate_cache($config, $current_dir);
        }
        if (isset($options['clear-global-cache'])) {
            self::clear_global_cache($config);
        }
        $providers = self::init_providers($options, $config, $current_dir);
        $stdout_report_options = self::init_stdout_report_options($options, $show_info, $output_format, $in_ci);
        ...
        $project_analyzer = new Project_Analyzer($config, $providers, $stdout_report_options, Project_Analyzer::get_file_report_options($report_file_paths, isset($options['report-show-info']) ? $options['report-show-info'] !== 'false' && $options['report-show-info'] !== '0' : true), $threads, $scan_threads, $progress);
        Cli_Utils::init_php_version($options, $config, $project_analyzer);
        $start_time = microtime(true);
        self::configure_project_analyzer($options, $config, $project_analyzer, $find_references_to, $find_unused_code, $find_unused_variables, $run_taint_analysis);
        if ($config->run_taint_analysis || $run_taint_analysis) {
            $is_diff = false;
        }
        /** @var string $plugin_path */
        foreach ($plugins as $plugin_path) {
            $config->add_plugin_path($plugin_path);
        }
        if ($paths_to_check === null) {
            $project_analyzer->check($current_dir, $is_diff);
        } elseif ($paths_to_check) {
            $project_analyzer->check_paths($paths_to_check);
        }
        if ($find_references_to) {
            $project_analyzer->find_references_to($find_references_to);
        }
        self::store_flow_graph($options, $project_analyzer);
        if (isset($options['generate-json-map']) && is_string($options['generate-json-map'])) {
            self::store_type_map($providers, $config, $options['generate-json-map']);
        }
        if (isset($options['generate-stubs'])) {
            self::generate_stubs($options, $providers, $project_analyzer);
        }
        if (!isset($options['i'])) {
            Issue_Buffer::finish($project_analyzer, !$paths_to_check, $start_time, isset($options['stats']), self::init_baseline($options, $config, $current_dir, $path_to_config, $paths_to_check));
        } else {
            self::auto_generate_config($project_analyzer, $current_dir, $init_source_dir, $vendor_dir);
        }
    }
    /** @return int<1, max> */
    public static function get_threads(array $options, Config $config, bool $in_ci, bool $for_scan): int
    {
        ...
        if ($for_scan) {
            if (isset($options['scan-threads'])) {
                ...
                $threads = max(1, (new Cpu_Core_Counter())->get_count());
            }
        } else if (isset($options['threads'])) {
    ...
    private static function init_output_format(array $options): string
    {
        return isset($options['output-format']) && is_string($options['output-format']) ? $options['output-format'] : self::find_default_output_format();
    }
    /**
     * @return Report::TYPE_*
     */
    private static function find_default_output_format(): string
    {
    ...
    private static function init_show_info(array $options): bool
    {
     ...
     * @param array<int, string> $args
     */
    private static function generate_config(string $current_dir, array &$args): void
    {
        ...
        $args = array_values(array_filter($args, static fn(string $arg): bool => $arg !== '--ansi' && $arg !== '--no-ansi' && $arg !== '-i' && $arg !== '--init' && $arg !== '--debug' && $arg !== '--debug-by-line' && $arg !== '--debug-emitted-issues' && !str_starts_with($arg, '--disable-extension=') && !str_starts_with($arg, '--root=') && !str_starts_with($arg, '--r=')));
        $init_level = null;
        ...
        if (null !== $init_level) {
            try {
                $template_contents = Creator::get_contents($current_dir, $init_source_dir, $init_level, $vendor_dir);
            } catch (Config_Creation_Exception $e) {
    ...
    private static function load_config(?string $path_to_config, string $current_dir, string $output_format, ?Class_Loader $first_autoloader, bool $run_taint_analysis, array $options): Config
    {
        $config = Cli_Utils::initialize_config($path_to_config, $current_dir, $output_format, $first_autoloader, $run_taint_analysis);
        if (isset($options['error-level']) && is_numeric($options['error-level'])) {
            ...
            if (!in_array($config_level, [1, 2, 3, 4, 5, 6, 7, 8], true)) {
                throw new Config_Exception('Invalid error level ' . $config_level);
            }
            $config->level = $config_level;
        }
        return $config;
    }
    private static function init_progress(array $options, Config $config, bool $in_ci): Progress
    {
        ...
        if ($debug) {
            ...
            if (isset($options['long-progress'])) {
                $progress = new Long_Progress($show_errors, $show_info, $in_ci);
            } else {
                $progress = new Default_Progress($show_errors, $show_info, $in_ci);
            }
        }
    ...
    private static function init_providers(array $options, Config $config, string $current_dir): Providers
    {
        if ($config->cache_directory === null || isset($options['i'])) {
            return new Providers(new File_Provider(), new Parser_Cache_Provider($config, Composer::get_lock_file($current_dir), false), new File_Storage_Cache_Provider($config, Composer::get_lock_file($current_dir), false), new Class_Like_Storage_Cache_Provider($config, Composer::get_lock_file($current_dir), false), new File_Reference_Cache_Provider($config, Composer::get_lock_file($current_dir), false));
        }
        ...
        return new Providers(new File_Provider(), new Parser_Cache_Provider($config, Composer::get_lock_file($current_dir), !$no_file_cache), new File_Storage_Cache_Provider($config, Composer::get_lock_file($current_dir), !$no_reflection_cache), new Class_Like_Storage_Cache_Provider($config, Composer::get_lock_file($current_dir), !$no_reflection_cache), new File_Reference_Cache_Provider($config, Composer::get_lock_file($current_dir), !$no_reference_cache), new Project_Cache_Provider());
    }
    /**
     * @param array{"set-baseline": mixed, ...} $options
     * @return array<string,array<string,array{o:int, s: list<string>}>>
     */
    private static function generate_baseline(array $options, Config $config, string $current_dir, ?string $path_to_config): array
    {
        fwrite(STDERR, 'Writing error baseline to file...' . PHP_EOL);
        $error_baseline = is_string($options['set-baseline']) ? $options['set-baseline'] : $config->error_baseline ?? Config::DEFAULT_BASELINE_NAME;
        try {
            $issue_baseline = Error_Baseline::read(new File_Provider(), $error_baseline);
        } catch (Config_Exception) {
            $issue_baseline = [];
        }
        Error_Baseline::create(new File_Provider(), $error_baseline, Issue_Buffer::get_issues_data(), $config->include_php_versions_in_error_baseline || isset($options['include-php-versions']));
        fwrite(STDERR, "Baseline saved to {$error_baseline}.");
        if ($error_baseline !== $config->error_baseline) {
            Cli_Utils::update_config_file($config, $path_to_config ?? $current_dir, $error_baseline);
        }
     ...
     * @return array<string,array<string,array{o:int, s: list<string>}>>
     */
    private static function update_baseline(array $options, Config $config): array
    {
        $baseline_file = $config->error_baseline;
        if (empty($baseline_file)) {
            fwrite(STDERR, 'Cannot update baseline, because no baseline file is configured.' . PHP_EOL);
            exit(1);
        }
        try {
            $issue_current_baseline = Error_Baseline::read(new File_Provider(), $baseline_file);
            $total_issues_current_baseline = Error_Baseline::count_total_issues($issue_current_baseline);
            $issue_baseline = Error_Baseline::update(new File_Provider(), $baseline_file, Issue_Buffer::get_issues_data(), $config->include_php_versions_in_error_baseline || isset($options['include-php-versions']));
            $total_issues_updated_baseline = Error_Baseline::count_total_issues($issue_baseline);
            ...
            fwrite(STDERR, 'Could not update baseline file: ' . $exception->get_message() . PHP_EOL);
            exit(1);
        }
        return $issue_baseline;
    }
    private static function store_type_map(Providers $providers, Config $config, string $type_map_location): void
    {
        $file_map = $providers->file_reference_provider->get_file_maps();
        $name_file_map = [];
        $expected_references = [];
        ...
        $reference_dictionary = Reference_Map_Generator::get_reference_map($providers->classlike_storage_provider, $expected_references);
        $type_map_string = json_encode(['files' => $name_file_map, 'references' => $reference_dictionary], JSON_THROW_ON_ERROR);
        $providers->file_provider->set_contents($type_map_location, $type_map_string);
    }
    private static function auto_generate_config(Project_Analyzer $project_analyzer, string $current_dir, ?string $init_source_dir, string $vendor_dir): void
    {
        $issues_by_file = Issue_Buffer::get_issues_data();
        if (!$issues_by_file) {
            $init_level = 1;
        } else {
            $codebase = $project_analyzer->get_codebase();
            $mixed_counts = $codebase->analyzer->get_total_type_coverage($codebase);
            $init_level = Creator::get_level(array_merge(...array_values($issues_by_file)), array_sum($mixed_counts));
        }
    ...
    private static function init_stdout_report_options(array $options, bool $show_info, string $output_format, bool $in_ci): Report_Options
    {
    ...
    private static function clear_global_cache(Config $config): never
    {
        $cache_directory = $config->get_global_cache_directory();
        if ($cache_directory) {
    ...
    private static function clear_cache(Config $config): never
    {
    ...
    private static function consolidate_cache(Config $config, string $current_dir): never
    {
        $cache_directory = $config->get_cache_directory();
        if ($cache_directory !== null) {
            $lock = Composer::get_lock_file($current_dir);
            (new Parser_Cache_Provider($config, $lock))->consolidate();
            (new File_Storage_Cache_Provider($config, $lock))->consolidate();
            (new Class_Like_Storage_Cache_Provider($config, $lock))->consolidate();
            (new File_Reference_Cache_Provider($config, $lock))->consolidate();
        }
        echo 'Cache consolidated' . PHP_EOL;
        exit;
    }
    private static function get_current_dir(array $options): string
    {
        $cwd = getcwd();
        if (false === $cwd) {
    ...
    private static function restart(array $options, bool $force_jit, int $threads, int $scan_threads, Progress $progress): void
    {
        $ini_handler = new Psalm_Restarter('PSALM');
        $ini_handler->enable_jit = $force_jit;
            ...
            if (is_array($options['disable-extension'])) {
                ...
                foreach ($options['disable-extension'] as $extension) {
                    if (is_string($extension)) {
                        $ini_handler->disable_extension($extension);
                    }
                }
            } elseif (is_string($options['disable-extension'])) {
                $ini_handler->disable_extension($options['disable-extension']);
            }
        }
        if (($threads > 1 || $scan_threads > 1) && extension_loaded('grpc') && (ini_get('grpc.enable_fork_support') === '1' && ini_get('grpc.poll_strategy') === 'epoll1') === false) {
            $ini_handler->disable_extension('grpc');
            $progress->warning(PHP_EOL . 'grpc extension has been disabled. ' . 'Set grpc.enable_fork_support = 1 and grpc.poll_strategy = epoll1 in php.ini to enable it. ' . 'See https://github.com/grpc/grpc/issues/20250#issuecomment-531321945 for more information.' . PHP_EOL . PHP_EOL);
        }
        ...
        if (function_exists('opcache_get_status')) {
            if (true === (opcache_get_status()['jit']['on'] ?? false)) {
                $has_jit = true;
                $progress->write(PHP_EOL . 'JIT acceleration: ON' . PHP_EOL . PHP_EOL);
            } elseif ($force_jit) {
                $progress->write(PHP_EOL . 'JIT acceleration: OFF (an error occurred while enabling JIT)' . PHP_EOL . 'Please report this to https://github.com/vimeo/psalm with your OS and PHP configuration!' . PHP_EOL . PHP_EOL);
            } else {
                $progress->write(PHP_EOL . 'JIT acceleration: OFF' . PHP_EOL . 'You can enable JIT acceleration (experimental) with --force-jit.' . PHP_EOL . PHP_EOL);
            }
        } else {
            $progress->write(PHP_EOL . 'JIT acceleration: OFF (opcache not installed or not enabled)' . PHP_EOL . 'Install and enable the opcache extension to use JIT with --force-jit.' . PHP_EOL . PHP_EOL);
        }
        if ($force_jit && !$has_jit) {
        ...
        if ($overcommit === '2') {
            $err = 'ERROR: VM overcommiting is disabled.' . PHP_EOL . PHP_EOL . "TL;DR: to fix, run these two commands:" . PHP_EOL . PHP_EOL . "echo 1 | sudo tee /proc/sys/vm/overcommit_memory" . PHP_EOL . "echo vm.overcommit_memory=1 | sudo tee /etc/sysctl.d/40-psalm.conf   # For persistence" . PHP_EOL . PHP_EOL . "Explanation: disabling VM overcommitting *WILL* cause failures when running Psalm " . "in multithreaded mode during analysis," . PHP_EOL . 'as Psalm relies very heavily on the copy-on-write semantics of fork(), which are currently disabled.' . PHP_EOL . PHP_EOL . PHP_EOL . "Please enable VM overcommitting to greatly speed up Psalm and avoid crashes in multithreaded mode." . PHP_EOL . PHP_EOL . PHP_EOL . "This warning may be ignored by setting the PSALM_IGNORE_NO_OVERCOMMIT=1 environment variable " . "(not recommended)." . PHP_EOL . PHP_EOL;
            fwrite(STDERR, $err);
            if (getenv('PSALM_IGNORE_NO_OVERCOMMIT') !== '1') {
                exit(1);
            }
        }
        Preloader::preload($progress, $has_jit);
    }
    /** @param array<int, string> $argv */
    private static function forward_cli_call(array $options, array $argv): void
    {
     ...
     */
    private static function init_config(string $current_dir, array $args, string $vendor_dir, ?Class_Loader $first_autoloader, ?string $path_to_config, string $output_format, bool $run_taint_analysis, array $options): array
    {
        $init_source_dir = null;
        if (isset($options['i'])) {
            self::generate_config($current_dir, $args);
            ...
            Creator::create_bare_config($current_dir, $init_source_dir, $vendor_dir);
            $config = Config::get_instance();
            $config->set_composer_class_loader($first_autoloader);
        } else {
            $config = self::load_config($path_to_config, $current_dir, $output_format, $first_autoloader, $run_taint_analysis, $options);
        }
     ...
     * @return array<string,array<string,array{o:int, s: list<string>}>>
     */
    private static function init_baseline(array $options, Config $config, string $current_dir, ?string $path_to_config, ?array $paths_to_check): array
    {
        $issue_baseline = [];
        if (isset($options['set-baseline'])) {
            if ($paths_to_check !== null) {
            ...
            $issue_baseline = self::generate_baseline($options, $config, $current_dir, $path_to_config);
        }
        ...
        if (isset($options['update-baseline'])) {
            ...
            $issue_baseline = self::update_baseline($options, $config);
        }
        if (!$issue_baseline && $baseline_file_path && !isset($options['ignore-baseline'])) {
            try {
                $issue_baseline = Error_Baseline::read(new File_Provider(), $baseline_file_path);
            } catch (Config_Exception $exception) {
                fwrite(STDERR, 'Error while reading baseline: ' . $exception->get_message() . PHP_EOL);
                exit(1);
            }
        }
        if ($paths_to_check !== null) {
    ...
    private static function store_flow_graph(array $options, Project_Analyzer $project_analyzer): void
    {
        /** @var string|null $dump_taint_graph */
        $dump_taint_graph = $options['dump-taint-graph'] ?? null;
        $flow_graph = $project_analyzer->get_codebase()->taint_flow_graph;
        if ($flow_graph !== null && $dump_taint_graph !== null) {
            file_put_contents($dump_taint_graph, "digraph Taints {\n\t" . implode("\n\t", array_map(static fn(array $edges): string => '"' . implode('" -> "', $edges) . '"', $flow_graph->summarize_edges())) . "\n}\n");
        }
    }
    /** @return false|'always'|'auto' */
    private static function should_find_unused_code(array $options, Config $config): bool|string
    {
    ...
    private static function should_run_taint_analysis(array $options): bool
    {
     ...
     */
    private static function configure_project_analyzer(array $options, Config $config, Project_Analyzer $project_analyzer, string|bool|null $find_references_to, false|string $find_unused_code, bool $find_unused_variables, bool $run_taint_analysis): void
    {
        if (isset($options['generate-json-map']) && is_string($options['generate-json-map'])) {
            $project_analyzer->get_codebase()->store_node_types = true;
        }
        ...
        if ($find_references_to !== null) {
            $project_analyzer->get_codebase()->collect_locations();
            $project_analyzer->show_issues = false;
        }
        if ($find_unused_code) {
            $project_analyzer->get_codebase()->report_unused_code($find_unused_code);
        }
        if ($config->find_unused_variables || $find_unused_variables) {
            $project_analyzer->get_codebase()->report_unused_variables();
        }
        if ($config->literal_array_key_check) {
            $project_analyzer->get_codebase()->literal_array_key_check = true;
        }
        $project_analyzer->get_codebase()->all_constants_global = $config->all_constants_global;
        $project_analyzer->get_codebase()->all_functions_global = $config->all_functions_global;
        if ($config->run_taint_analysis || $run_taint_analysis) {
            $project_analyzer->track_tainted_inputs();
        }
        if ($config->find_unused_psalm_suppress || isset($options['find-unused-psalm-suppress'])) {
            $project_analyzer->track_unused_suppressions();
        }
    }
    private static function configure_shepherd(Config $config, array $options, array &$plugins): void
    {
    ...
    private static function generate_stubs(array $options, Providers $providers, Project_Analyzer $project_analyzer): void
    {
        if (isset($options['generate-stubs']) && is_string($options['generate-stubs'])) {
            $stubs_location = $options['generate-stubs'];
            $providers->file_provider->set_contents($stubs_location, Stubs_Generator::get_all($project_analyzer->get_codebase(), $providers->classlike_storage_provider, $providers->file_storage_provider));
        }
    }
    /**
     * @psalm-pure
     */
    private static function get_help_text(): string
    {
        $formats = [];
        /** @var string $value */
        foreach ((new ReflectionClass(Report::class))->get_constants() as $constant => $value) {
            if (str_starts_with($constant, 'TYPE_')) {
                $formats[] = $value;
            }
        }
        sort($formats);
        $output_formats = wordwrap(implode(', ', $formats), 75, "\n            ");
        /** @psalm-suppress ImpureMethodCall */
        $reports = array_keys(Report::get_mapping());
        sort($reports);
        $report_formats = wordwrap('"' . implode('", "', $reports) . '"', 75, "\n        ");
        ...
        Reports:
            --report=PATH
                ...
                (Currently supported formats: {$report_formats})
