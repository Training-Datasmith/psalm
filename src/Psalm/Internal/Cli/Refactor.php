...
final class Refactor
{
    ...
    public static function run(array $argv): void
    {
        ...
        $valid_long_options = ['help', 'debug', 'debug-by-line', 'debug-emitted-issues', 'config:', 'root:', 'scan-threads:', 'threads:', 'move:', 'into:', 'rename:', 'to:'];
        ...
        (new Xdebug_Handler('PSALTER'))->check();
        ...
        $progress = $debug ? new Debug_Progress() : new Default_Progress();
        if (array_key_exists('debug-emitted-issues', $options)) {
            $config->debug_emitted_issues = true;
        }
        $project_analyzer = new Project_Analyzer($config, $providers, new Report_Options(), [], $threads, $scan_threads, $progress);
        if (array_key_exists('debug-by-line', $options)) {
            $project_analyzer->debug_lines = true;
        }
        $project_analyzer->refactor_code_after_completion($to_refactor);
