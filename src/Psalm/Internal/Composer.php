...
final class Composer
{
     ...
     */
    public static function get_json_file_path(string $root): string
    {
     ...
     */
    public static function get_lock_file_path(string $root): string
    {
        $composer_json_path = self::get_json_file_path($root);
    ...
    public static function get_lock_file(string $root): string
    {
        if (self::$lock_file !== null) {
            return self::$lock_file;
        }
        $root = self::get_lock_file_path($root);
        if (file_exists($root) && is_readable($root)) {
            $root = Providers::safe_file_get_contents($root);
        } else {
            $root = '';
        }
        return self::$lock_file = $root;
    }
}