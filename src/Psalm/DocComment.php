...
final class Doc_Comment
{
     ...
     */
    public static function parse_preserving_length(Doc $docblock, bool $no_psalm_error = false): Parsed_Docblock
    {
        ...
        foreach ($parsed_docblock->tags as $special_key => $_) {
            ...
            if (str_starts_with((string) $special_key, 'psalm-')) {
                ...
                if (!in_array($special_key, self::PSALM_ANNOTATIONS, true)) {
                    throw new Docblock_Parse_Exception('Unrecognised annotation @psalm-' . $special_key);
                }
            }
        }
        return $parsed_docblock;
     ...
     */
    public static function parse_suppress_list(string $suppress_entry): array
    {
        preg_match('/
                (?(DEFINE)
